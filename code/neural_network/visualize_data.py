...
#     magnitude = np.linalg.norm(u_values_np, axis=0)
    
#     u_img = axs[0].imshow(magnitude, cmap='viridis', norm=Normalize(vmin=np.min(magnitude), vmax=np.max(magnitude)))    # origin='lower'
#     axs[0].set_title('Velocity Magnitude')
#     fig.colorbar(u_img, ax=axs[0], fraction=0.046, pad=0.04)

#     if len(p_values_np.shape) == 3:
#         p_values_np = p_values_np.squeeze(0)
        
#     p_img = axs[1].imshow(p_values_np, cmap='plasma', norm=Normalize(vmin=np.min(p_values_np), vmax=np.max(p_values_np)))
#     axs[1].set_title('Pressure Field')
#     fig.colorbar(p_img, ax=axs[1], fraction=0.046, pad=0.04)

#     domain_max = 0.006
#     n_ticks = 5
#     tick_locations = np.linspace(0, u_values_np.shape[1]-1, n_ticks)
