...
if __name__ == '__main__':
    ...
    data_losses, physics_losses = train_model(model, train_loader, optimizer, criterion, num_epochs, device, alpha=alpha,
                                              model_save_path=model_save_path, loss_save_path=loss_save_path, log_interval=log_interval, is_physics_informed=True)

    # - - - - - Testing - - - - -
    
    avg_loss, results = test_model(model, test_loader, criterion, device, test_loss_save_path=loss_save_path, log_interval=log_interval, is_physics_informed=True)    
